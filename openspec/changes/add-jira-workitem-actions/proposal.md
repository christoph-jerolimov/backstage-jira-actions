...
- New backend plugin `jira-actions` (workspace package `plugins/jira-actions-backend`, plugin ID `jira`) built on the new backend system.
...
- Backend wiring: the plugin is added to `packages/backend/src/index.ts`, and `backend.actions.pluginSources` in `app-config.yaml` gains the `jira` entry so the actions are discoverable (and therefore exposed over MCP).
...
- **Modified code**: `packages/backend/package.json` + `packages/backend/src/index.ts` (register the plugin), `app-config.yaml` (`backend.actions.pluginSources` gains `jira`; example `connections` entry documented).
