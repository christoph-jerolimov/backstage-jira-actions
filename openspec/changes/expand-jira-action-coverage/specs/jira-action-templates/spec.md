...
- **THEN** the form asks for the action's inputs (issueType and summary as required; projectKey, entityRef, description, descriptionFormat, labels, assignee, parentKey, customFields, host as optional, with either projectKey or entityRef expected)
...
- **WHEN** a user runs any of the list-returning test templates (`list-projects`, `list-issue-types`, `search-work-items`, `search-users`, `list-transitions`, `list-link-types`, `list-fields`, `list-boards`, or `list-sprints`)
